...
public class MainAppController {
    ...
    private String generarResumenParaDialogo() {
        ...
        // --- CÁLCULO VISUAL DEL DESCUENTO (33%) ---
        if (view.getCmbVentaFormaPago().getSelectedItem().equals("Efectivo")) {
            BigDecimal descuento = subtotalTotal.multiply(new BigDecimal("0.33"));
            BigDecimal totalFinal = subtotalTotal.subtract(descuento);
            
            sb.append(String.format("%-31s %15.2f\n", "DESCUENTO (33%):", descuento.negate())); // Negativo visual
            sb.append(String.format("%-31s %15.2f\n", "TOTAL A PAGAR:", totalFinal));
            sb.append("\nForma de Pago: Efectivo.");
        } else {
            // CREDITO
            sb.append(String.format("%-31s %15.2f\n", "TOTAL A PAGAR:", subtotalTotal));
