...
public class DepositoPanel extends JPanel {
    ...
    public DepositoPanel(MainFrame app) {
        this.app = app;
        setOpaque(false);
        setLayout(new GridBagLayout());

        // Tamaño compacto fijo
        ...
        JButton ok = Styles.primary("Depositar");
        JButton back = Styles.ghost("← Regresar");
        ok.addActionListener(e -> depositar());
        back.addActionListener(e -> app.showCard(MainFrame.MENU));
        buttons.add(ok); buttons.add(back);

        c.gridx = 0; c.gridy++; c.gridwidth = 2; c.fill = GridBagConstraints.NONE; c.weightx = 0;
        form.add(buttons, c);

        // Para que el formulario quede arriba y no se estire
    ...
    private void depositar() {
        ...
        try {
            ...
            boolean ok = api.deposito(cta, imp);
            JOptionPane.showMessageDialog(this, ok ? "Depósito realizado." : "No se pudo depositar.");
        } catch (Exception ex) {
